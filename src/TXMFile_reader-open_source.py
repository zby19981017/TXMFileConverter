...
class OleReader:
    ...
    def ReadTxm(self):
        ...
        for i in range(len(self.img_dir)):
            imgdata_temp = int(self.img_dir[i][0][9:])
            img_temp = int(self.img_dir[i][1][5:])
            if (imgdata_temp > ImageData_max):
                ImageData_max = imgdata_temp
            if (img_temp > Image_max):
                Image_max = img_temp
        img_dir3 = []
        for i in range(1, Image_max + 1):
            flag_count = int((i - 1) / 100) + 1
            S1 = 'ImageData' + str(flag_count)
            S2 = 'Image' + str(i)
            img_dir3 = np.append(img_dir3, [S1, S2])

        img_dir3 = img_dir3.reshape(Image_max, 2)
        self.img_dir = img_dir3
...
def getargv():
    ...
    OutputSetting.add_argument('I_height', metavar='Resized model height', default=100, type=int,
                               help="Please set bigger than 0")
    OutputSetting.add_argument('I_depth', metavar='Resized model depth', default=200, type=int,
                               help="Please set bigger than 0")
